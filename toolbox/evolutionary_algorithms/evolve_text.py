...
def levenshtein_distance(s1, s2, d = dict()):
    ...
    if (len(s1), len(s2)) in d:
        return d[(len(s1), len(s2))]
    else:
        d[(len(s1), len(s2))] = min([int(s1[0] != s2[0]) + levenshtein_distance(s1[1:],s2[1:], d), 1+levenshtein_distance(s1[1:],s2, d), 1+levenshtein_distance(s1,s2[1:], d)])
        return d[(len(s1), len(s2))]
...
def mutate_text(message, prob_ins=0.05, prob_del=0.05, prob_sub=0.05):
    ...
    if random.random() < prob_ins:
        ...
        rand_int = random.randint(0, len(message))
        message = message[:rand_int] + [random.choice(VALID_CHARS)] + message[rand_int:]
        if message[0] == " ":
            message = message[1:]
    ...
    if random.random() < prob_del:
        rand_int = random.randint(0, len(message)-1)
        message.pop([rand_int])

    if random.random() < prob_sub:
        rand_int = random.randint(0, len(message)-1)
        message[rand_int] = random.choice(VALID_CHARS)
        while message[0] == " ":
            message[0] = random.choice(VALID_CHARS)
